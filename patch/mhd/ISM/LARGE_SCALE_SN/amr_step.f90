...
!     end if
  ...
  !this line is for the restart
  if(t_last_sn .eq. 0.) t_last_sn = t 
 
  !---------------------
  ! Supernova explosions
  !---------------------
  !feedback not linked to sink particles
  if(supernovae .and. ilevel == levelmin .and. .not.(feedback_sink)) then
     if(sn_freq_mult .eq. 0.) then 
       if(t >= sn_time(sn_i) .and. .not. sn_all) then
           if(myid == 1) write (*,*) 'Supernova, t =', t, 'sn_time =', sn_time(sn_i)
           call make_sn
!           if(tracer) call create_sn_tracer
  ...
  !feedback emanating from sink particles
  if(supernovae .and. ilevel == levelmin .and. feedback_sink) then
     call make_sn_sink
  endif
