...
subroutine make_sn
  ...
  if(sn_freq_mult .eq. 0.) then
     ...
     sn_cent2_all(1) = sn_center(sn_i, 1)
     sn_cent2_all(2) = sn_center(sn_i, 2)
     sn_cent2_all(3) = sn_center(sn_i, 3)
...
!     sn_r = 2.*(0.5**levelmin)*scale
     ...
     !no correlation between density and supernovae
     if ( .not. (dens_corr) ) then 

       call random_number(xseed)
       sn_cent2_all(1)= xseed*boxlen
       call random_number(xseed)
       sn_cent2_all(2)= xseed*boxlen
       ...
       sn_cent2_all(3)=(0.5*boxlen + sign*xseed1*sqrt(-2./xx2*log(xx2)) *  Height0) !assume mid-plane for the moment 
     endif
...
!  call cmp_cpumap(x,cc,1)
  ...
  !correlation between density peak and supernovae
  if(dens_corr) then 
...
!  write(*,*) 'my id', myid, 'dens_max', dens_max_v(myid,1), dens_max_v(myid,2), dens_max_v(myid,3), dens_max_v(myid,4)
  ...
  !end on the dens_corr conditions, i.e. correlation between density peak and supernovae
...
!  write(*,*) 'put a SN in ',myid,n_sn
  ...
  if(dens_max_all .gt. dens_thres .or. .not.(dens_corr) ) then 
