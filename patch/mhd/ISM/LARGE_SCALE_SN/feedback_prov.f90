...
!note the gaussian distribution is not used here. It is kept for memory reason as it is fast
...
!   write(*,*) '1 myid ', myid, 'x_sn, y_sn, z_sn, ',sn_cent(1),sn_cent(2),sn_cent(3)

  ! Loop over levels to look for the density maximum
...
!find the densest cells amongst all cpu
  dens_max_s=dens_max
  call MPI_ALLREDUCE(dens_max_s,dens_max_all,3,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
...
!  if( myid .eq. cc(1) .or. myid .eq. 1 ) write(*,*) 'myid ',myid ,sn_cent2_all(1),sn_cent2_all(2),sn_cent2_all(3)
...
!  write(*,*) 'put a SN in ',myid,n_sn
        ...
        do i = 1, ngrid
          if(ok(i)) then
              ...
              if(rr < 1.) then
                ...
                if(dgas .gt. dens_max) dens_max = dgas
                ...
                pgas = sqrt(0.01*sn_ed / dgas) * dgas !!more a guess for now one should compute the momentum
                ...
                if(once .and. myid .eq. cc(1)) then
                   write(*,*) '3 put SN, n_sn, myid ',myid, n_sn, 'x,y,z: ',sn_cent2_all(1),sn_cent2_all(2),sn_cent2_all(3), 'density ',dgas !, uold(ind_cell(i),1),uold(ind_cell(i),2),uold(ind_cell(i),3),uold(ind_cell(i),4),uold(ind_cell(i),5),ekin,rr
                   once=.false.
                endif
  ...
  call MPI_ALLREDUCE(dens_max,dens_max_all,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,info)
#endif
...
!  if(myid .eq. cc(1)) write(*,*) '4 n_sn ', n_sn_all
  ...
  if(myid .eq. 1) write(102,112) t,sn_cent2_all(1),sn_cent2_all(2),sn_cent2_all(3),dens_max_all,mass_sn_tot_all

  112 format(6e12.4)

!endif !end of the if on the second loop
