subroutine init_hydro
  ...
  !open a file to write supernovae
  if(myid .eq. 1) open(102,file='supernovae.txt',form='formatted',status='unknown',access='append')
  if(nrestart .eq. 0 .and. myid .eq. 1) then
     write(102,*) 't ','sn_x ','sn_y ','sn_z ','dens_max ','mass '  
  endif


  if(myid .eq. 1) open(103,file='supernovae2.txt',form='formatted',status='unknown',access='append')
  if(nrestart .eq. 0 .and. myid .eq. 1) then
     write(103,*) 't ','sn_x ','sn_y ','sn_z ','dens_max ','mass ', 'isink ', 'msink ', 'msink_last_feed '  
  endif
