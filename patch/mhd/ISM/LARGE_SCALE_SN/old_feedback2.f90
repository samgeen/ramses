subroutine make_sn
  ...
  real(dp),dimension(1:ndim):: sn_cent,sn_cent2
  real(dp), dimension(1:nvector, 1:ndim), save:: xx
  real(dp):: sn_r, sn_m, sn_e, sn_vol, sn_d, sn_ed, sn_rp, dx_sel
  real(dp):: rr, pi,dens_max
  logical:: sel = .false.
  ...
  if(sn_freq_mult .eq. 0.) then
     ...
     call random_number(xseed)
     sn_cent(3)=(0.5*boxlen + (xseed-0.5) * 2. *  Height0) !assume mid-plane for the moment 
  endif
...
!  if(myid .eq. 1) then
!   write(*,*) 'myid ', myid, 'x_sn, y_sn, z_sn, ',sn_cent(1),sn_cent(2),sn_cent(3)
!  endif
    ...
    do igrid = 1, ncache, nvector
      ...
      do ind = 1, twotondim
        ...
        do i = 1, ngrid
          ...
          if( uold(ind_cell(i), 1) .gt. dens_max .and. abs(xx(i,3)-boxlen*0.5) .lt. Height0 ) then 
             sn_cent2(1) = xx(i,1)
             sn_cent2(2) = xx(i,2)
             sn_cent2(3) = xx(i,3)
...
!  write(*,*) 'myid',myid, 'n_sn ', n_sn, 'once ',once

 ! now if we are in the right CPU, put the supernovae at the densest place
 if(n_sn .gt. 50*(ilevel/levelmin)**3) then 
        ...
        do i = 1, ngrid
          if(ok(i)) then
              ...
              if(rr < 1.) then
                ...
                if(once) then
!                   write(*,*) 'put SN, n_sn, myid ',myid, n_sn, 'x,y,z: ',sn_cent2(1),sn_cent2(2),sn_cent2(3)
                   once=.false.
                endif
