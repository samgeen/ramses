...
!  if (myid .eq.1) write(*,*) 'nsink ',nsink
...
subroutine make_sink(ilevel)
  ...
  if (myid .eq.1) write(*,*)' Entering make_sink for level ',ilevel
  ...
  !ph introduced by PH. New sink should be far enough to old ones.
  !rsink_max as defined originally is not used
  rsink_max= 10.*(0.5**ilevel)  !(rsink_max*3.08d21/scale_l)**2
...
!  if (myid .eq.1) write(*,*)' Entering d_sink ',d_sink
...
!       if(dmaxi .gt. n_sink) write(*,*) 'dmaxi ',dmaxi, d_thres,n_sink
...
!  rmax=dble(ir_cloud)*dx_min ! Linking length in physical units
  !changed by PH
  dx_min=scale*0.5D0**levelmin
  rmax=dx_min*dble(ir_cloud)/2.
...
!     end do
  ...
  do isink=1,nsink
     ...
     !introduced by PH 09/2013 to follow the feedback from the sinks
     msink_last_feed_new(igrp)=msink_last_feed_new(igrp)+msink_last_feed(isink)
  ...
  !introduced by PH 09/2013 to follow the feedback from the sinks
  msink_last_feed(1:nsink)=msink_last_feed_new(1:nsink)
