     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              CASE ('hlld')
                  if( ( (qright(4)**2+qright(6)**2+qright(8)**2)/qright(2) .gt. switch_solv .or.  (qleft(4)**2+qleft(6)**2+qleft(8)**2)/qleft(2) .gt. switch_solv  ) .or. ( qleft(1)/qright(1) .gt. 3.) .or. (qleft(1)/qright(1) .lt. 0.3  ) )  then
                   CALL lax_friedrich(qleft,qright,fgdnv,zero_flux) 
                  else
        ...
        do j = jlo, jhi
           do i = ilo, ihi+1 ! WARNING HERE
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,6,2)*dsgn .le. 0 .or. dq(l,i-1,j,k,6,2)*dsgn .le. 0.) then
                   dbf(l,i,j,k,1,1)=0.
                 else
                 dbf(l,i,j,k,1,1) = dsgn*min(abs(dbf(l,i,j,k,1,1)),abs(dq(l,i,j,k,6,2)),abs(dq(l,i-1,j,k,6,2)))
                 endif
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,6,3)*dsgn .le. 0 .or. dq(l,i-1,j,k,6,3)*dsgn .le. 0.) then
                   dbf(l,i,j,k,1,2)=0.
                 else
                 dbf(l,i,j,k,1,2) = dsgn*min(abs(dbf(l,i,j,k,1,2)),abs(dq(l,i,j,k,6,3)),abs(dq(l,i-1,j,k,6,3)))
                 endif
        ...
        do j = jlo, jhi+1 ! WARNING HERE
           do i = ilo, ihi
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,7,1)*dsgn .le. 0 .or. dq(l,i,j-1,k,7,1)*dsgn .le. 0.) then
                   dbf(l,i,j,k,2,1)=0.
                 else
                 dbf(l,i,j,k,2,1) = dsgn*min(abs(dbf(l,i,j,k,2,1)),abs(dq(l,i,j,k,7,1)),abs(dq(l,i,j-1,k,7,1)))
                 endif
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,7,3)*dsgn .le. 0 .or. dq(l,i,j-1,k,7,3)*dsgn .le. 0.) then
                   dbf(l,i,j,k,2,2)=0.
                 else
                   dbf(l,i,j,k,2,2) = dsgn*min(abs(dbf(l,i,j,k,2,2)),abs(dq(l,i,j,k,7,3)),abs(dq(l,i,j-1,k,7,3)))
                 endif
        ...
        do j = jlo, jhi
           do i = ilo, ihi
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,8,1)*dsgn .le. 0 .or. dq(l,i,j,k-1,8,1)*dsgn .le. 0.) then
                   dbf(l,i,j,k,3,1)=0.
                 else
                 dbf(l,i,j,k,3,1) = dsgn*min(abs(dbf(l,i,j,k,3,1)),abs(dq(l,i,j,k,8,1)),abs(dq(l,i,j,k-1,8,1)))
                 endif
              ...
              do l = 1, ngrid
                 ...
                 !!ph 11/06/2010 try to be more consistent with the slope
                 if(dq(l,i,j,k,8,2)*dsgn .le. 0 .or. dq(l,i,j,k-1,8,2)*dsgn .le. 0.) then
                   dbf(l,i,j,k,3,2)=0.
                 else
                   dbf(l,i,j,k,3,2) = dsgn*min(abs(dbf(l,i,j,k,3,2)),abs(dq(l,i,j,k,8,2)),abs(dq(l,i,j,k-1,8,2)))
                 endif
