  ...
  if(ilevel.eq.levelmin)then
     ...
     if(cosmo)then
        ...
        if(aexp_next.gt.aout(iout))then
           ...
           dt_want = (dt_want - t)/nsubcycle(ilevel)*1.001
           dtnew(ilevel)=min(dtnew(ilevel),dt_want) 
        endif
     else
        dt_want = (tout(iout)-t)/nsubcycle(ilevel)*1.00001
        dtnew(ilevel)=min(dtnew(ilevel),dt_want)
     endif
  endif
