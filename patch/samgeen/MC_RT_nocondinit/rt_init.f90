...
!-------------------------------------------------------------------------
  ...
  use UV_module
  implicit none
  integer:: i, k, ilevel, ivar, nvar_count
!-------------------------------------------------------------------------
  ...
  ! STG HACK
  ! Update the RT source to take account of the new boxlen
  if(rt) then
     do k=1,rt_nsource
        rt_src_x_center(k) = rt_src_x_center(k) * boxlen
        rt_src_y_center(k) = rt_src_y_center(k) * boxlen
        rt_src_z_center(k) = rt_src_z_center(k) * boxlen
        write(*,*) "RT_SRC_X/Y/Z_CENTER(k)", rt_src_x_center(k) &
             & , rt_src_y_center(k), rt_src_z_center(k)
     enddo
  endif
  ! STG HACK ENDS
